...
async def help(ctx):
    if ctx.guild.id == default_config.get("StaffGuildID"):
      ...
      help1.add_field(name="**{}logs <uid>**".format(prefix), inline=False, value="Get modmail logs for a user.")
      ...
      await ctx.send("This command only works in the staff guild. If you are a user who wants to use the bot!Thank U.")
